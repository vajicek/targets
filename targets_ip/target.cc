...
std::optional<cv::Vec3f> Target::get_target_point(
	...
	auto binorm = normal.cross(up);
	auto point3 = center + binorm * point[0] + up * point[1];
...
void sample_space(const Target &target,
	...
	const std::function<void(const Vec2i &, const std::optional<Vec3f> &)>
		&process_sample) {
	...
	for (int y = 0; y < count[1]; y++) {
		for (int x = 0; x < count[0]; x++) {
			...
			const Vec3f direction =
				cv::normalize(Vec3f{projection_plane_cooord[0],
...
void sample_image(const Target &target,
	...
	const std::function<void(
		const Vec2i &, const std::optional<Vec3f> &, const Vec3f &)>
	...
	sample_space(target,
		...
		[&camera_image_float, &process_image_sample](
			const Vec2i &int_coord, const std::optional<Vec3f> &model_color) {
			auto camera_image_color_float =
				camera_image_float.at<Vec3f>(int_coord[1], int_coord[0]);
			process_image_sample(
				int_coord, model_color, camera_image_color_float);
		});
}
...
void visualize_metric(const cv::Mat &camera_image,
	...
	sample_image(target,
		...
		[&metric_samples, &field_height](const Vec2i &int_coord,
			...
			metric_samples.at<Vec3f>(int_coord[1], int_coord[0]) =
				camera_image_color_float;
			if (model_color.has_value()) {
				auto diff =
					camera_image_color_float - 255 * model_color.value();
				auto dist = std::sqrt(diff.dot(diff));
				if ((int_coord[0] / field_height +
						int_coord[1] / field_height) %
						2 ==
					0) {
					metric_samples.at<Vec3f>(int_coord[1], int_coord[0]) =
						Vec3f{dist};
